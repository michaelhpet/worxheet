...
flowchart TB
    subgraph TAURI["Tauri Process"]
        ...
        subgraph BE["Backend (Rust /core)"]
            subgraph PIPELINE["pipeline.rs (command cores)"]
...
- **pipeline.rs** is the orchestration layer: each `#[tauri::command]` has a
  testable `run_*` core (`core/src/pipeline.rs`). Heavy work is wrapped in
